...
def test(lib,
         ...
         k_cache_stride=None,
         v_cache_stride=None):
    print(f"Testing Attention on {torch_device} with n_q_head:{n_q_head} n_kv_head:{n_kv_head} seq_len:{seq_len} head_dim:{head_dim} pos:{pos} "
    ...
    k_cache = (torch.rand([n_kv_head, k_cache_buf_len, head_dim], dtype=dtype).to(torch_device) * 0.1)
    v_cache = (torch.rand([n_kv_head, v_cache_buf_len, head_dim], dtype=dtype).to(torch_device) * 0.1)
    ...
    check_error(lib.infiniopCreateAttentionDescriptor(handle,
    ...
    check_error(lib.infiniopGetAttentionWorkspaceSize(descriptor, ctypes.byref(workspace_size)))
