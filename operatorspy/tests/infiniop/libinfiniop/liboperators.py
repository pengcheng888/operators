...
def open_lib():
    ...
    if system_name == "Windows":
        ...
        libop_path = find_library_in_ld_path("lib", "libinfiniop_operators.so")
        librt_path = find_library_in_ld_path("lib", "libinfiniop_operators.so")

    assert (libop_path is not None), f"Cannot find infiniop.dll or libinfiniop.so. Check if INFINI_ROOT is set correctly."
    assert (librt_path is not None), f"Cannot find infinirt.dll or libinfinirt.so. Check if INFINI_ROOT is set correctly."
    print()

    print("librt_path: ", librt_path)  ## librt_path:  /home/ubuntu/.infini/lib/libinfinirt.so
    print("libop_path: ", libop_path)  ## librt_path:  /home/ubuntu/.infini/lib/libinfiniop.so
    print()
    ...
    lib.infiniopCreateTensorDescriptor.argtypes = [POINTER(infiniopTensorDescriptor_t),
                                                   c_uint64,
                                                   POINTER(c_uint64),
                                                   POINTER(c_int64),
                                                   DataLayout]
